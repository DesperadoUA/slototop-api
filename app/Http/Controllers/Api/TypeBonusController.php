...
class TypeBonusController extends PostController
{
    ...
    public function show($id)
    {
        ...
        if(!$data->isEmpty()) {
            ...
            $response['body']['posts'] = [];
            $arr_posts = Relative::getPostIdByRelative($this->tables['BONUS_TYPE_BONUS_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
    ...
    public function category($id){
        ...
        $settings = [
            'table' => $this->tables['TYPE_BONUS'],
            'table_meta' => $this->tables['TYPE_BONUS_META'],
            'table_category' => $this->tables['TYPE_BONUS_CATEGORY'],
            'table_relative' => $this->tables['TYPE_BONUS_CATEGORY_RELATIVE']
        ];
        ...
        if(!$data->isEmpty()) {
            ...
            $response['body']['posts'] = [];
            $arr_posts = Relative::getPostIdByRelative($this->tables['TYPE_BONUS_CATEGORY_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
                $post = new Posts(['table' => $this->tables['TYPE_BONUS'], 'table_meta' => $this->tables['TYPE_BONUS_META']]);
