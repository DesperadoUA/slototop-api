...
class TechnologyController extends PostController
{
    ...
    public function show($id)
    {
        ...
        if(!$data->isEmpty()) {
            ...
            $arr_posts = Relative::getPostIdByRelative($this->tables['CASINO_TECHNOLOGY_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
    ...
    public function category($id){
        ...
        $settings = [
            'table' => $this->tables['TECHNOLOGY'],
            'table_meta' => $this->tables['TECHNOLOGY_META'],
            'table_category' => $this->tables['TECHNOLOGY_CATEGORY'],
            'table_relative' => $this->tables['TECHNOLOGY_CATEGORY_RELATIVE']
        ];
        ...
        if(!$data->isEmpty()) {
            ...
            $response['body']['posts'] = [];
            $arr_posts = Relative::getPostIdByRelative($this->tables['TECHNOLOGY_CATEGORY_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
                $post = new Posts(['table' => $this->tables['TECHNOLOGY'], 'table_meta' => $this->tables['TECHNOLOGY_META']]);
