...
class LanguageController extends PostController
{
    ...
    public function show($id)
    {
        ...
        if(!$data->isEmpty()) {
            ...
            $arr_posts = Relative::getPostIdByRelative($this->tables['CASINO_LANGUAGE_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
    ...
    public function category($id){
        ...
        if(!$data->isEmpty()) {
            ...
            $response['body']['posts'] = [];
            $arr_posts = Relative::getPostIdByRelative($this->tables['LANGUAGE_CATEGORY_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
                $post = new Posts(['table' => $this->tables['LANGUAGE'], 'table_meta' => $this->tables['LANGUAGE_META']]);
