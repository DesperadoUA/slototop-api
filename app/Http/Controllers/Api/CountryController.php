...
class CountryController extends PostController
{
    ...
    public function show($id)
    {
        ...
        if(!$data->isEmpty()) {
            ...
            $arr_posts = Relative::getPostIdByRelative($this->tables['CASINO_COUNTRY_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
    ...
    public function category($id){
        ...
        $settings = [
            ...
            'table_meta' => $this->tables['COUNTRY_META'],
            'table_category' => $this->tables['COUNTRY_CATEGORY'],
            'table_relative' => $this->tables['COUNTRY_CATEGORY_RELATIVE']
        ];
        ...
        if(!$data->isEmpty()) {
            ...
            $response['body']['posts'] = [];
            $arr_posts = Relative::getPostIdByRelative($this->tables['COUNTRY_CATEGORY_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
                $post = new Posts(['table' => $this->tables['COUNTRY'], 'table_meta' => $this->tables['COUNTRY_META']]);
