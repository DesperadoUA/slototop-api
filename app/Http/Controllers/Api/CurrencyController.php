...
class CurrencyController extends PostController
{
    ...
    public function show($id)
    {
        ...
        if(!$data->isEmpty()) {
            ...
            $response['body']['posts'] = [];
            $arr_posts = Relative::getPostIdByRelative($this->tables['PAYMENT_CURRENCY_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
    ...
    public function category($id){
        ...
        if(!$data->isEmpty()) {
            ...
            $response['body']['posts'] = [];
            $arr_posts = Relative::getPostIdByRelative($this->tables['CURRENCY_CATEGORY_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
                $post = new Posts(['table' => $this->tables['CURRENCY'], 'table_meta' => $this->tables['CURRENCY_META']]);
