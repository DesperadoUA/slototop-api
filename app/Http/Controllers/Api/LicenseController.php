...
class LicenseController extends PostController
{
    ...
    public function show($id)
    {
        ...
        if(!$data->isEmpty()) {
            ...
            $arr_posts = Relative::getPostIdByRelative($this->tables['CASINO_LICENSE_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
    ...
    public function category($id){
        ...
        $settings = [
            'table' => $this->tables['LICENSE'],
            'table_meta' => $this->tables['LICENSE_META'],
            'table_category' => $this->tables['LICENSE_CATEGORY'],
            'table_relative' => $this->tables['LICENSE_CATEGORY_RELATIVE']
        ];
        ...
        if(!$data->isEmpty()) {
            ...
            $response['body']['posts'] = [];
            $arr_posts = Relative::getPostIdByRelative($this->tables['LICENSE_CATEGORY_RELATIVE'], $data[0]->id);
            if(!empty($arr_posts)) {
                $post = new Posts(['table' => $this->tables['LICENSE'], 'table_meta' => $this->tables['LICENSE_META']]);
